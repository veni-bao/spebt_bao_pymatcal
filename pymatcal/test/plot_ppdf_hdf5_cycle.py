...
def plot_ppdf_hdf5_cycle(configFname):
    # det_idx = int(sys.argv[1])
    config = pymatcal.get_config(configFname)
    det_idx = 0
    common_fname=re.match('^(.+).yml',configFname).group(1)
    ...
    for angidx in range(60):
        ...
        mtrans = pymatcal.get_mtransform(
            -config['angle'], img_dims[0]*0.5, det_dimy*0.5)
        xy = pymatcal.coord_transform((mtrans[0], mtrans[1]), config['det geoms'][:, (0, 2, 4)]
                                +[+config['dist']+img_dims[0]*0.5,0,0]-mtrans[1])


        det_list = [mpl.patches.Rectangle(xy,geom[1] - geom[0], geom[3] - geom[2], angle=config['angle'],
                            rotation_point=(xy[0], xy[1]))
                    for xy, geom in zip(xy, config["det geoms"])
                    ]

        act_det_xy = pymatcal.coord_transform((mtrans[0], mtrans[1]), config['active dets'][:, (0, 2, 4)]
                                +[config['dist']+img_dims[0]*0.5,0,0]-mtrans[1])
        act_det_list = [mpl.patches.Rectangle(xy,
    ...
    # out_fname = data_fname_base + '_data_det_%03d.png'%det_idx
        out_fname = data_fname_base + '_data_det_%03d.png'%det_idx+str(angidx+1)
    # xlim_det = np.array([np.min(xy[:, 0]), np.max(config["det geoms"][:, 1])])
        ylim_det = np.array([np.min(xy[:, 1]), np.max(
        config["det geoms"][:, 3] + [det_dimy*0.5,det_dimy*0.5,det_dimy*0.5])])
